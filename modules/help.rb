module SerieBot
    module Help
        ...
        command(:help, description: 'Display a list of commands.') do |event|
            ...
            Commands.image_commands.each do |name, _file|
                imagecoms += "`#{Config.prefix}#{name}`, "
            end
            text1 << imagecoms + "\n"

            textcoms = ''
            text1 << "__Text Commands:__\n"
            Commands.text_commands.each do |name, _file|
                textcoms += "`#{Config.prefix}#{name}`, "
            end
            text1 << textcoms + "\n\n"
            event.user.pm(text1)
            ...
            Images.folderimage_commands.each do |name, _file|
                foldcoms += "`#{Config.prefix}#{name}`, "
            end
            ...
            text3 << "`#{Config.prefix}ban`: Permanently Ban somebody from the server. | Usage: `#{Config.prefix}ban @User\#1234 <reason>`\n"
